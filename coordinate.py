...
class GoToCoordinate:
    def __init__(self):
        rospy.init_node('go_to_coordinate', anonymous=True)
        self.pub = rospy.Publisher('/cmd_vel', Twist, queue_size=10)
        self.sub = rospy.Subscriber('/odom', Odometry, self.odom_callback)
        self.rate = rospy.Rate(10)  # 10 Hz
        self.target_position = Point(1, 0, 0)
    ...
    def move_to_target(self):
        ...
        distance = math.sqrt((self.target_position.x - self.current_position.x) ** 2 +
                             (self.target_position.y - self.current_position.y) ** 2)

        # Calculate the angle to the target position
        desired_angle = math.atan2(self.target_position.y - self.current_position.y,
        ...
        # If the robot is far from the target, move towards it
